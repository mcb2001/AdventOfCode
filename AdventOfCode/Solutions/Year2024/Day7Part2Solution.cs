...
{
    [Skippable]
    internal class Day7Part2Solution : Year2024Solution
    {
        ...
        private static IEnumerable<long> PotentialValues(long[] values)
        {
            if (values.Length == 2)
            {
                ...
                yield return values[0] * values[1];
                yield return Concanate(values[0], values[1]);
            }
            else
            {
                foreach (long value in PotentialValues(values[..^1]))
                {
                    ...
                    yield return value * values[^1];
                    yield return Concanate(value, values[^1]);
                }
            }
        }

        private static long Concanate(long a, long b)
